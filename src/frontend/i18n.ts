...
const messages = {
  ...
  'login.defaultPassword': { zh: '默认密码', en: 'Default password' },
  'login.forgotPassword': { zh: '忘记密码？', en: 'Forgot password?' },
  'login.envHint': { zh: '管理员密码通过环境变量 ADMIN_PASSWORD 配置', en: 'Admin password is set via the ADMIN_PASSWORD env variable' },
  ...
  'pwd.envIntro': { zh: '管理员密码通过环境变量 ADMIN_PASSWORD 控制，请在 Cloudflare Dashboard 中修改：', en: 'Admin password is controlled by the ADMIN_PASSWORD environment variable. Update it in Cloudflare Dashboard:' },
  'pwd.step1': { zh: '打开 Cloudflare Dashboard → Workers & Pages → cf-shorturl', en: 'Open Cloudflare Dashboard → Workers & Pages → cf-shorturl' },
  'pwd.step2': { zh: '进入 Settings → Variables', en: 'Go to Settings → Variables' },
  'pwd.step3': { zh: '编辑或重新创建 ADMIN_PASSWORD（勾选 Secret 类型）', en: 'Edit or recreate ADMIN_PASSWORD (check Secret type)' },
  'pwd.step4': { zh: '重新部署后新密码立即生效', en: 'Redeploy for the new password to take effect' },
  'pwd.note': { zh: '注意：密码修改后需重新部署 Worker（推送代码或手动在 Dashboard 重新部署）。', en: 'Note: Redeploy the Worker after changing the password (push code or redeploy manually in Dashboard).' },
