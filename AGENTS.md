...
**存储依赖:** 通过 Service Binding 调用同账号下的 [cf-kv-filesystem](https://github.com/joyanhui/cf-kv-filesystem) Worker（部署名 `kv-filesystem`），自身不依赖任何 KV / D1 / R2。
...
- **存储**: 通过 Service Binding 调用 kv-filesystem（部署名），自身无 KV/D1/R2 绑定
- **缓存策略**:
